...
for i = 1:length(SNR)
    ...
    for a = 1:4
        ...
        H = taps;%LS_estimator(pilot_modulated,channel_array_out,SNR(i));

        if (a==3)           %ZF-Equalization
            equalized = zf_equalizer(channel_array_out,H);
        elseif (a==4)       %MMSE-Equalization
            equalized = MMSE(channel_array_out,H,SNR(i));
        else                %No Equalization
            equalized = channel_array_out;
        end

        parallel = reshape(equalized,1,N_sub,[]);
